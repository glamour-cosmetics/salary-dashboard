...
export default function TopBar({ title, subtitle, avatarUrl, period, onPeriodClick, onBack }) {
  return (
    <header className="sticky top-0 z-50 bg-slate-50/80 backdrop-blur-xl flex justify-between items-center w-full px-6 py-4">
      <div className="flex items-center gap-3 min-w-0">
        {onBack ? (
          <button
            onClick={onBack}
            className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-container-high transition-colors text-on-surface"
          >
        ...
        <div className="flex flex-col min-w-0">
          ...
          {subtitle && <span className="text-[10px] uppercase tracking-widest font-semibold text-slate-500 truncate">{subtitle}</span>}
        </div>
      </div>
      {onPeriodClick && (
        <button
