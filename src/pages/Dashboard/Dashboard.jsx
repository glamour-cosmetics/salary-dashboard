...
export default function Dashboard() {
  ...
  const planPct = mockKPI.planAchievement
  ...
  return (
    <div className="min-h-screen bg-surface pb-32">
      <TopBar
        ...
        subtitle={mockEmployee.position}
          ...
          <div className="bg-gradient-to-br from-primary-container to-primary p-8 rounded-xl shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl md:text-5xl font-extrabold text-white tracking-tighter">
                    {formatCurrency(mockSalary.total, '')}
                  </span>
                  <span className="text-lg md:text-2xl font-medium text-on-primary-container">{mockSalary.currency}</span>
                </div>
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-lg px-4 py-2 border border-white/10 inline-flex items-center gap-2 self-start md:self-auto">
                ...
                <span className="text-white text-xs font-bold">{mockSalary.trend}</span>
              </div>
            </div>
          </div>
        </section>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-secondary">{formatPercent(mockKPI.planAchievement)}</p>
                  </div>
                </div>
              ...
              <div className="bg-surface-container-lowest p-4 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-secondary">{formatPercent(mockKPI.acb.achievement)}</p>
                  </div>
                </div>
                <span className="material-symbols-outlined text-secondary">arrow_upward</span>
              </div>
        ...
        <section className="bg-surface-container-lowest p-6 rounded-xl border-l-4 border-tertiary-fixed-dim">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-lg font-bold text-on-surface tracking-tight">
                Next Goal: +{formatCurrency(mockKPI.overachievementSteps.nextGoalBonus)} bonus
              </h2>
          ...
          <div className="flex justify-between items-end mb-6">
            ...
            <p className="text-xl font-bold text-on-surface tracking-tighter">
              {formatCurrency(mockKPI.overachievementSteps.salesGap, '')}
              <span className="text-xs font-medium text-outline ml-1 uppercase">{mockSalary.currency}</span>
            </p>
          </div>
          <button
